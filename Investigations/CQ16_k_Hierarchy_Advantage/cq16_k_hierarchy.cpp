 ...
 * @brief CQ16: The k-Hierarchy Advantage
 *
...
{
    std::puts("================================================================");
    std::puts("  CQ16: THE k-HIERARCHY ADVANTAGE");
...
static void part1_hierarchy()
{
    ...
    for (auto& b : bodies) {
        ...
        char tag[32];
        std::snprintf(tag, sizeof(tag), "CQ16-I.%s", b.name);
        char desc[128];
...
static void part2_koppa_identity()
{
    ...
    prove("CQ16-II.1", "koppa_hydrogen = r_e (exact identity)",
          koppa_H, r_e, 0.01);

    // Verify against laws.hpp
    prove("CQ16-II.2", "bridge::koppa_hydrogen matches alpha^2*a_0",
    ...
    prove("CQ16-II.3", "alpha * lambda_bar_C = r_e (intermediate check)",
          chain_2, r_e, 0.01);
}
static void part3_G_uncertainty()
{
    ...
    prove("CQ16-III.1", "G relative uncertainty = 22 ppm",
          G_rel * 1e6, 22.0, 5.0);
...
static void part4_cross_scale()
{
    ...
    prove("CQ16-IV.1", "koppa_proton = koppa_hydrogen = r_e",
    ...
    std::puts("    This is confirmed by CQ09 — the kinematic map reproduces");
    ...
    prove("CQ16-IV.3", "R_p from W+1 conjecture (0.02%)",
    ...
    prove("CQ16-IV.4", "Solar rotation from v (order of magnitude)",
...
static void part5_observables()
{
    ...
    prove("CQ16-V.1", "k defined for proton (no GM available)",
...
static void part6_indictment()
{
    ...
    prove("CQ16-VI.1", "koppa*c^2 recovers GM_Sun exactly",
