# CQ37 Investigation Summary: Elementary Charge Quantisation from Lattice Topology

**Investigation ID**: CQ37  
**Title**: Elementary Charge Quantisation from Lattice Topology  
**Date**: June 2026  
**Status**: COMPLETE (Investigation phase)  
**Verdict**: INCONCLUSIVE — e remains Tier-1 primitive; topological derivation pending CQ36  
...
2. **Candidate 2**: Charge from topological linking number — **DEFERRED** (dimensionally correct, awaits CQ36)
3. **Candidate 3**: Charge from W=1 boundary radius via Coulomb identity — **FAILS** (circular, requires CQ03)

**Result**: Dimensional closure is mathematically impossible. Charge must be a topological quantisation, not dimensional scaling. Further derivation requires CQ36 (linking-number computation) or CQ03 (α derivation from topology).
...
**Verdict**: FAILS — Circular. To use this candidate, CQ03 must first derive α from topology (marked as hard open problem with unknown effort).
...
| 2 | e₀ × √Lk | ✓ PASS | Pending | Pending | DEFERRED (CQ36) |
...
**Finding 5**: Path forward requires either CQ36 (linking-number computation, moderate effort) or CQ03 (α derivation, unknown hard effort).
...
2. Candidate 2 requires CQ36 completion (uncertain timeline)
...
### Option A: Topological Derivation (CQ36 + Linking Number)
**If pursuing removal of e from whitelist**:

1. **CQ36**: Compute Lk(hydrogen) from vortex topology
...
### Option B: Do NOT Pursue CQ03 Path
**Avoid attempting to derive α from topology**:

Reasons:
1. CQ03 explicitly marks circularity as "deep" and "fundamental"
...
Candidate 2: [SDT-derived] × [Pending]         = Class B (if CQ36 succeeds)
...
Compute the topological linking number between electron (W=1 torus) and proton (W=3 trefoil knot) in hydrogen ground state. If successful, enables Candidate 2 derivation for CQ37 charge quantisation.

**Dependency**: Must complete CQ36 before CQ37 can pursue topological path.
...
- CQ03: Fine Structure from Topology (circularity analysis)
- CQ36: Linking-Number Computation (pending)
...
**Next Step**: Pursue Option A (CQ36) or accept Option B (e as Tier-1 primitive)
