...
**Constraints (derivation basis only).** Inputs limited to `{ℓ_P, c, ℏ, k_B, T_CMB, α, m_e, m_p}` + measured masses and the measured `B/A` curve **as an OBSERVED-TARGET anchor only** (AME2020). Use the curve's *data* (Fe-56/Ni-62 near the maximum) as the anchor; **never** import the semi-empirical-mass-formula *story* (its five fitted terms) into the SDT chain — at most it appears as an OBSERVED comparison column. **Prohibited:** G/M/GM, wavefunctions, fields-as-primitives, quarks/gluons, virtual particles as objects, dark matter/energy, ΛCDM, duality, and the liquid-drop/SEMF fitted coefficients as inputs. Never write "charge radius" or treat charge as a substance (use "handed redirection"; `e` = unit-bridge).
