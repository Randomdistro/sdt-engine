...
All constants from `sdt::laws::measured` (CODATA 2018). Topology from `PPT01` (vortex equilibrium), `PPT05` (trefoil confinement), `PPT06` (spation traction), `PPT07` (mass-ratio), and `nuclear.hpp` (grammar).
...
| measured anchors | `m_n=939.5654`, `m_p=938.2720`, `m_e=0.5110` MeV; `m_n−m_p=1.2933 MeV`; `Q_n=0.7824 MeV`; `τ_n`: bottle 877.75 s / beam 887.7 s (Δ≈9.5 s puzzle); `B(t)=8.482`, `B(³He)=7.718 MeV`; tritium `Q_β=18.591 keV`, `t½=12.32 yr` | | `NP11`, `NP14` |
...
- **A free open strand costs `E(L)=σL → ∞`** (`PPT05` Theorem B): every strand must re-close. This is *why nothing is ever a bare half-vortex*.
...
**Prediction.** `μ_n / μ_p = −2/3` exactly from the (unknot-loaded trefoil) vs (bare trefoil) poloidal-current ratio (cross-check `NP15`).
...
**Premise.** `τ_n` is the **mean time to execute the crossing-change** that unlinks the dyad — a topological barrier crossing, not a random clock. From `PPT05`, an open strand re-closes on `t_collapse = a/c`; the *forward* unlink is the rare process.
...
```
**Q-C1.** Can `τ_n ≈ 880 s` be reached from `f_circ` and a barrier `B_link` expressed in the *same* `σ_string`/`P_conv/3` units as `PPT05`?
**Q-C2.** **The beam–bottle discrepancy (Δ ≈ 9.5 s, ~1%)**: is it a **branching** between two unlink channels — (i) full `p+e+ν̄` and (ii) a rare bound-state / radiative channel that a *beam* counts but a *bottle* does not (or vice-versa)? Predict the branching ratio `≈ Δτ/τ ≈ 1.0%` and its sign. (This is `NP11`'s falsifiable core.)
...
**Formulation.**
```
V_torus(p,q) = 2π² √(q/p) · a³                  [Pappus, PPT01/PPT03/CQ10b/PPT07]
...
**Q-D1.** Derive `a_p/a_e = 11.442` from the (2,3)-vs-(1,1) knot equilibrium (`PPT01`: `R/a=√(q/p)`, budget `v_p²+v_t²=c²`) — i.e. from the *minor-radius* that the trefoil's three-fold confinement forces relative to the unknot's. Target: 11.44 ± 1%.
...
**Q-G2.** Does the link-radius set `a₀`, and does the `1836` re-appear as the **`a₀ : R_p` lever** via the same `m_p/m_e` (the reciprocal-drive gear, `PPT06`: `ω_p/ω_e = 3(m_p/m_e)/α`)?
**Gate PASS-G.** The electron's emergence radius and Hopf re-link reproduce `a₀ = 5.29e-11 m` within the existing `FLM03`/`GOM02` koppa identities (`α²a₀ = r_e`).
...
`nuclear.hpp` (grammar, `h3_grammar`, triton as alpha-invalid primitive) · `laws.hpp::winding` (R_p, W_eff — **frozen, Stage E verifies**) · `laws.hpp::law_IV` (V_disp, R_excl ladder) · `PPT01` (R/a=√(q/p), budget) · `PPT05` (`E=σL`, open-strand collapse) · `PPT06` (reciprocal-drive gear) · `PPT07`/`PPT03`/`CQ10b` (Pappus torus volume) · `NP11` (neutron lifetime) · `NP14` (n–p mass diff) · `NP15` (nucleon moments) · the visualizer (Stage G electron Hopf-thread).
