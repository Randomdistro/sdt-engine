...
## Questions This Opens *(generative — log new ones in `OP01_VERDICT.md`)*

These are *not* required for the verdict; they are the payoff if OP01 lands well.

1. **Is the matter index `n` derivable without any lab anchor at all?** If bound-vortex `z` gives
   1.333 to 5%, does the *same* electropause depth predict the indices of the noble gases and the
   crown/flint glass split — i.e. is `n(material)` a readout of bound-vortex closure depth, period?
2. **Does the 1.75″ ↔ GD06 tie collapse to a single integral?** If the solar eikonal of Phase 3 and
   GD06's `4ϟ/b` are the *same* line of code, is the lensing/refraction split purely linguistic — and
   can the entire GD06 cluster be re-narrated as gradient-index optics with no metric language?
3. **Is `n = 1/(1−z)` exact or the leading term of an octave law?** CR07 makes `1+z = 2^{Δn}`; does
   the index then read `n = 2^{Δn_local}` in strong fields, recovering `1/(1−z)` only for `z≪1`, and
   does that fix the `z≥1` break (Phase 1 pivot) natively rather than piecewise?
4. **Is dispersion `n(λ) = n₀ + β(ℓ_P/λ)²` a genuine vacuum prediction?** If `β` is DERIVED (not
   calibrated), SDT predicts a tiny vacuum birefringence/dispersion at fixed `λ` — does any pulsar
   dispersion-measure or GRB timing bound rule it in or out?
5. **Does the temp-minimum fold (Phase 4 → OP07) survive a real `T(h)`?** Is the caustic a robust
   single-surface signature, or does it wash out once the measured chromospheric `T(h)` replaces the
   schematic profile — i.e. is OP07's capstone falsifier actually sharp?
