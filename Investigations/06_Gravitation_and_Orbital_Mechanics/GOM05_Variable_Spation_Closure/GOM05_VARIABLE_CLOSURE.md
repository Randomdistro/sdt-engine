# GOM05: Variable Spation Closure, the Light-Speed Gradient & Lumiostasis
...
> **Builds on:** GOM01 (depth engine, Shapiro), FLM06 (spation closure ℓ_P=√(ϟ·ƛ)), GOM12 (pressure domain)
...
**Measured (GOM05-I.1, I.2 — both 0.0000%):** at the solar surface
...
This collapses GOM01 and FLM06 into one statement: the Shapiro "depth" was never
...
**Measured (GOM05-III.1, 0.0000%):** with `L_Sun = 3.828×10²⁶ W` and
...
## Shapiro, re-read (measured, GOM05-IV.1, 0.0000%)

```
Δt = (2ϟ/c) ln(4 r₁ r₂ / b²) = 123.66 µs   (Earth–Mars, b = R_Sun)
```
Identical number to the standard formula — but the **mechanism** is the GOM05
...
- The c-gradient identities (z = closure deficit = c deficit) are **exact** and
  parameter-free given ϟ — a genuine unification of GOM01 + FLM06.
- The **absolute** value of ℓ_P,∞ still carries the FLM06 seed caveat (one anchor
  unavoidable); GOM05 fixes the *variation* ℓ_P(r)/ℓ_P,∞ = 1−z, which is fully
