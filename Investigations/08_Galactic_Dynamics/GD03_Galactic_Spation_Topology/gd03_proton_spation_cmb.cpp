...
//     → Ratio r_e / R_p = m_p / m_e × α² / (6π⁵) [derived in PPT07]
//
//   Lattice occupation:
...
static void section_B_proton_lattice() {
    ...
    printf("   r_e / R_p = %.6f  (= 1/alpha × m_e/m_p × 6pi^5 from PPT07)\n",
           ratio_re_Rp);
    // From PPT07: m_p/m_e = 6pi^5 ≈ 1836.118
    double mp_me = 6.0 * std::pow(M_PI, 5.0);
    double predicted_ratio = (1.0 / alpha) * (1.0 / mp_me);
    printf("   PPT07 prediction: %.6f   deviation: %.4e\n\n",
...
int main() {
    ...
    printf("   A. CMB → ontic rate: f_ontic = k_B T_CMB / h = %.4e Hz\n",
           2.701 * k_B * T_CMB / h);
    printf("   B. Proton boundaries: Ϟ(r_e) = r_e exactly. PPT07 ratio confirmed.\n");
