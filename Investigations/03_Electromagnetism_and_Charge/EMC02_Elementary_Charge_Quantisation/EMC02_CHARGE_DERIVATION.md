...
> **Outcome**: e remains Tier-1 input; path forward identified (PPT09 + topology)
...
**Motivation**: PPT02 proved that α (fine structure constant) requires e as an input. But **where does e come from?** Is it a fundamental primitive (irreducible), or a topological quantum of the spation lattice?
...
This candidate is dimensionally correct and physically well-motivated, but requires topological computation. PPT09 (Linking-Number Computation for Hydrogen) must be completed first.
...
- To use this candidate, PPT02's circularity must be resolved (which PPT02 explicitly marked as an open problem)
...
### Option A: Complete PPT09 and Pursue Linking-Number Derivation

**Procedure**:
1. **PPT09**: Compute linking number Lk(hydrogen) from vortex topology
...
### Option B: Solve PPT02 and Derive α from Topology

**Procedure**:
1. **PPT02**: Derive $\alpha = 1/137.036$ from W=1 torus geometry without circularity
...
**Effort**: **UNKNOWN** — PPT02 explicitly states this is a hard open problem. Likely months to years.

**Probability of Success**: Low (PPT02 already attempted; circularity is deep)
...
**Outcome**: EMC02 concludes with **Class X** classification (Tier-1 input, no further reduction possible without topology).
...
| **2** | Linking Number | ✓ Correct | Pending | Pending | PPT09 | **DEFERRED** |
| **3** | W=1 Boundary | ✓ Correct | ✓ Match | ✗ Circular | PPT02 | **FAILS** |
...
| Candidate 2 | SDT-derived | Pending | B* | Success if PPT09 completes |
| Candidate 3 | Calibrated | Internal-only | E | Circular (requires PPT02) |
...
1. **PPT09 + Linking Number**: Derive $e$ from topological linking invariant (dimensionless) and lattice charge quantum $e_0$
2. **PPT02 Completion**: Derive $\alpha$ from topology, then use coulomb identity
3. **Accept as Primitive**: Keep $e$ in whitelist (permissible under CLAUDE.md)

**Recommended next step**: Prioritise **Option A (PPT09)** because:
