# EMC02 Verdict: Elementary Charge Quantisation

**Investigation**: EMC02 — Elementary Charge Quantisation from Lattice Topology  
**Date**: June 2026  
...
2. **Candidate 2 (Linking Number)**: Dimensionally correct but computationally deferred (requires PPT09)
3. **Candidate 3 (W=1 Boundary via Coulomb Identity)**: Mathematically exact but circular (requires PPT02)

### Root Cause
**Dimensional closure is impossible.** The whitelist cannot produce dimension [C] = [A·s] through any algebraic combination. Charge must be a topological quantisation, not a dimensional scaling. Topological derivation requires PPT09 (linking number) or PPT02 (α derivation) to complete.

### Recommendation
**Keep e in the whitelist as a Tier-1 primitive.** CLAUDE.md permits "measured observables" as external inputs; e is measured (SI exact, 2019). Alternatively, pursue PPT09 for topological linking-number derivation (moderate probability).
...
This candidate passes dimensional analysis and is physically well-motivated (topological quantisation), but requires computation of linking number Lk(hydrogen) from vortex topology. PPT09 must be completed.
...
Answer: UNKNOWN — awaits PPT09 completion and lattice impedance analysis
```
...
**Consequence**: To use Candidate 3, α must be derived from topology (breaking the r_e definition). PPT02 explicitly marked this as an open problem with unknown effort.

**Conclusion**: **FAILS** — Circular. Requires solving PPT02 first.
...
### Finding 5: Path Forward Requires PPT09 or PPT02
- **PPT09** (Linking Number): Dimensionally viable, moderate effort, unknown outcome
- **PPT02** (Derive α): Circular problem explicitly unsolved; very high effort, low probability
...
2. Candidate 2 requires completion of PPT09 (uncertain timeline)
...
### Secondary Recommendation: Prioritise PPT09 (Linking Number Path)
**If topological derivation is desired**:
1. Complete PPT09: Compute Lk(hydrogen) from vortex topology
...
### Tertiary Recommendation: Do NOT Pursue PPT02 Path
**Rationale**:
1. PPT02 explicitly states circularity is "deep" and "fundamental"
...
- [x] Test Candidate 2 (Linking Number) — DEFERRED (awaits PPT09)
- [x] Test Candidate 3 (W=1 Boundary) — FAILS (circular)
- [x] Dimensional analysis — Closure impossible
- [x] Identify path forward — PPT09 or acceptance as primitive
...
**Audit Trail**: Class X (Tier-1 primitive); further reduction pending PPT09 or PPT02  
