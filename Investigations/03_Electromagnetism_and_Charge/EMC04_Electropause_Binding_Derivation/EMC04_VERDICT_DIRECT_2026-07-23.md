# EMC04 — Direct re-run verdict (2026-07-23)

> **Run class: DIRECT** (main session, no agents). Prior agent-era `EMC04_VERDICT.md` and
> `emc04_results.txt` deleted 2026-07-23 (git-recoverable). **`INVESTIGATION.md` KEPT** — Harvey's
> theory document (2026-06-18) with the honesty contract; its claimed numbers were treated as
> unverified and re-earned today. Tool: `emc04_electropause.cpp`, g++ 15.2 -O2, exit **0**.
> Output: `emc04_rerun_2026-07-23.txt`.

## Contraband check

**CLEAN.** The α-reading caveat is stated inside the doc itself (α as kinematic k-rung, not
Coulomb coupling); no wavefunction, no rival mechanism in the chain. The one borderline object,
`a₀`, is written in SDT primitives (ℏ/(m_e c α), verified 6e-10).

## What the re-run measured (all four checks re-earned directly)

| check | today's number | gate |
|---|---|---|
| binding = centripetal = Coulomb = occlusion at a₀ | ratios **1.0000000000** | PASS |
| P_eff inverted from electropause balance | 5.225210e31 Pa, rel err **4.1e-12** vs engine | PASS |
| closed form 4αℏc/(π R_p² r_e²) | rel err 6.1e-10 | PASS |
| whitelist form m_p²m_e²c⁵/(4παℏ³) | rel err **3.9e-4** (= W+1 R_p residual) | PASS |

The centripetal force was computed with **no charge term** (delete-test for charge passes); the
whitelist form contains no k_e and no e.

## Verdict

- **Prompt completion: A** · **Physics class: CONVERGENCE→DERIVED-on-framework-terms** —
  **P_eff: class E → class C**, now on **direct-run evidence** (the prior support was agent-era).
- **Named residuals (the honest whole of what remains):** (1) **α is the one input** — read as
  the H-rung v/c; the model does not derive α and says so. (2) The cleanest whitelist form rides
  the **conjectural W+1 rule** R_p = 4ℏ/(m_p c) at the 2e-4 level — same single dependency EMC01
  traced. A reader who insists α ≡ k_e e²/ℏc will call the chain circular; on SDT's own terms
  (α = koppa rung, PPT02) it is not. Both readings stated; neither hidden.
- **Recovery:** n/a · **Cascade root:** W+1 rule (Law VI).

*This is the "electropause derivation of P_eff" result, previously memory/agent-class — now
re-earned by direct execution. The campaign's Front-2 ledger row for P_eff may cite this file.*
