# CQ28D — Electron Threading Feasibility *(topological permission, with a real intersection test)*
...
**Inputs.** CQ28C nuclear topology (trefoil tubes at frozen CQ28B sites).
...
```
*(The v4 "trefoil electrons" used exactly those approximations — shell rings + 2n² + styling. That is feasibility **theatre**. CQ28D replaces it with the intersection solver.)*

**Reuses.** the Hopf-fibre + poloidal-channel construction; `CQ11` (no free open strand). **Provenance:** class **D** (novel feasibility prediction) if a real solver passes; **F** if only theatre.
