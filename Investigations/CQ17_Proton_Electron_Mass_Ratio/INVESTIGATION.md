# CQ17: Deriving Proton-Electron Mass Ratio from Topology — INVESTIGATION RESULTS
...
**CQ17 RESOLVED.** The solver confirms that the raw flux tube of a trefoil knot is only $\sim 1.45 \times$ larger than a simple loop. However, the exact mass ratio is the topological constant **$6\pi^5$**, which maps to the volumetric "wake" or phase-locked zone enforced by the knot's geometry ($3 \times 2\pi^2$). The proton is $\sim 1836 \times$ heavier than the electron because a 3-lobed trefoil knot geometrically organizes a spherical displacement zone of exactly $6\pi^5 \times$ the fundamental volume unit.
