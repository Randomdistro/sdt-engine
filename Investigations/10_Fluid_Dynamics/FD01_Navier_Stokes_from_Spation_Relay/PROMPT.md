...
*This investigation inherits the §0 anti-creep protocol and rules R0–R5 verbatim (derivation basis inputs only; no G/M/kg fundamentals; no fields/wavefunctions/quarks/ΛCDM; certification labels on every result; translation test on every borrowed term; honesty over success).*
...
- **Method.** Formalise the update operator `u_i(t+t_P) = R[{u_j(t)}, ∇P_i, δ_i]`. Tag every constant DERIVED / MEASURED-INPUT / CALIBRATED(n) in a parameter ledger (R2). Cite FLM02 (GPI), FLM05 (pulse propagation), ROOT-SIM (per-spation contact mechanics) as upstream.
...
- **FLM02 (Granular Pulse Mechanics)** — GPI, `F = ma`, Newton I–III at the cell level; source of the inertial term and the acceleration asymmetry `δ`.
- **FLM05 (Lattice Pulse Propagation)** — pulse propagation in the lattice; supplies the relay-update kinematics.
