...
```
FD01  Navier–Stokes from spation relay        ← the FD root (rests on ROOT-SIM, FLM02, FLM05)
  │
  ...
  │     └─ FD06  Boundary layer & no-slip        (from PPT06 traction; δ ∼ √(νx/U))
...
| **FD05** | Sound, Shocks & the c/√3 Ceiling | compression relay wave `c_s=√(dP/dρ)`; shock = super-relay disturbance | air 343 m/s; c/√3 limit (CR04); boom ≡ Cherenkov (E57) |
| **FD06** | Boundary Layer from Traction | PPT06 spation traction grips the wall ⇒ no-slip (derived, not imposed) | Blasius δ/x=5/√Re_x; C_f=0.664/√Re_x |
| **FD07** | Circulation, Lift & Magnus | asymmetric wake (PPT06 ℓ=2); bound Γ = differential traction | Kutta–Joukowski L=ρUΓ; thin-airfoil slope 2π; Magnus sign |
...
| FD05 | **A**/C | **c/√3** native from P=u/3 (zero params, = CR04 BAO); boom ≡ Cherenkov (E57) one-mechanism unification; air c_s +0.031% |
...
`law_VI::traction` (PPT06: `T=3(W+1)`, wake ℓ=1/2/3) · `law_VI::topology` · `law_VI::confinement`.
Prior results leaned on: **B16** (transport ν∝√T, exponent 0.5), **CR04** (c/√3 radiation-fluid
sound speed), **E57** (Cherenkov shock), **E59** (superfluid phase-locked vortices), **CQ41**
(gear-frequency binding), **FLM02/FLM05/ROOT-SIM** (the granular-relay root), and the
