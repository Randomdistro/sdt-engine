...
This is the same `c/√3` that CR04 used as the coupled-lattice (pre-Clearing) BAO sound speed; FD05 *derives* it as the high-stiffness asymptote (`w = P/(ρc²) = 1/3`) of the single relation `c_s = √(dP/dρ)`. It is a ceiling, not a floor: ordinary fluids sit far below it because their `w ≪ 1/3`.
...
- **Method:** evaluate `dP/d(ρ_eff c²) = 1/3` symbolically and numerically from `law_I::P_rad`; cross-check against the `c/√3` CR04 used for BAO. Show the ordinary-fluid `c_s` (Phase 1) sits at `w ≪ 1/3`, on the same `c_s(w) = c√w` curve.
- **Success:** `c_s = 1.732×10⁸ m/s` to 4 sig figs; equals CR04's coupled-lattice sound speed exactly; `c_s/c = 0.5774`.
...
- **Method:** tabulate `c_s(w)` for `w` from 10⁻¹² (cold gas) to 1/3 (radiation); overplot the air, water, and CR04 points; verify monotonic approach to the ceiling and that nothing exceeds `c/√3` for `w ≤ 1/3`.
...
**Upstream (required):** FD01 (Navier–Stokes / Euler continuum limit of the relay), FD02 (compressibility ↔ relay stiffness), **CR04** (`c_s = c/√3`, BAO coupled-lattice sound speed), **E57** (Cherenkov radiation as a super-phase-speed lattice shock, VERIFIED).
...
| F2 | Radiation-fluid ceiling | `c_s = c/√3 = 1.732×10⁸ m/s` from `P=u/3`, 4 sig figs | the c/√3 limit is not native to `√(dP/dρ)`; CR04 link broken |
...
- Plots (optional, gnuplot/matplotlib from `fd05_results.txt`): (i) `c_s` vs `w` on log–log with air/water/CR04 points and the `c/√3` saturation line; (ii) Mach angle `θ` vs `M`; (iii) overlay of the boom envelope and the E57 Cherenkov cone showing the shared geometry.
