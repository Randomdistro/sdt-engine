...
which matches the CQ09 formula to machine precision (error < 10⁻¹⁵).
...
> The per-contact lag coefficient a/r = ℓ_P/(2r) is derived from Planck-scale spation rolling mechanics (CQ35, CLASS A PASS). The macro velocity field v(r) = v₀√(R_p/r) is its rigorous consequence, validated to machine precision.
...
|--------|----------|--------|
| CQ02 | Trefoil (2,3) geometry, v_T, v_P | ✓ |
| CQ14 | Traction mechanism, v₀ = 1.8301c | ✓ |
| CQ09 | Target formula v(r) = v₀√(R_p/r) | ✓ |
...
|--------|----------|-------|
| CQ09 | Derived coefficient | Validates formula |
| CQ12 | Velocity field mechanics | Emission mechanism |
| CQ16 | Rotation at all scales | k-hierarchy, bonding |
...
This investigation closes the foundational gap between trefoil-scale circulation (CQ02, CQ14) and atomic-scale rotation fields (CQ09), placing SDT's rotation theory on solid mechanical ground.
