 ...
 * @brief CQ15: zk^2 = 1 Spation Depth Engine — Canonical Investigation
 *
...
// =====================================================================
//  PROOF INFRASTRUCTURE (CQ09 pattern)
// =====================================================================
...
{
    std::puts("================================================================");
    std::puts("  CQ15: zk^2 = 1 SPATION DEPTH ENGINE");
...
static void section_I_define_field()
{
    ...
    prove("CQ15-I.1a", "z(R_Sun) = 1/k_Sun^2",
    ...
    prove("CQ15-I.2a", "zk^2 = 1 for Sun (algebraic identity)",
          bridge::zk2_product(bridge::z_from_k(bridge::k_Sun), bridge::k_Sun),
          1.0, 1e-10);

    prove("CQ15-I.2b", "zk^2 = 1 for Earth",
    ...
    prove("CQ15-I.2c", "k >= 1 outside c-boundary (k at 2*koppa)",
    ...
    prove("CQ15-I.2d", "k < 1 inside c-boundary (superluminal)",
    ...
    prove("CQ15-I.2e", "koppa = GM/c^2 (GR bridge verification)",
...
static void section_II_redshift()
{
    ...
    prove("CQ15-II.1", "Pound-Rebka redshift (SDT vs GR prediction)",
          dnu_nu_sdt, dnu_nu_gr, 0.001);

    prove("CQ15-II.2", "Pound-Rebka redshift magnitude ~ 2.46e-15",
          dnu_nu_sdt, 2.46e-15, 0.5);

    // II.3: Sign check — photon climbing OUT of well loses energy (redshift)
    prove("CQ15-II.3", "Correct sign: z_bottom > z_top (redshift climbing up)",
    ...
    prove("CQ15-II.4", "Solar redshift: SDT matches GM/(c^2 R)",
...
static void section_III_shapiro()
{
    ...
    prove("CQ15-III.1", "Numerical integral matches closed form",
          dt_numerical, dt_closed, 0.1);

    prove("CQ15-III.2", "SDT closed form matches GR Shapiro formula",
    ...
    prove("CQ15-III.3", "Delay decreases with larger impact parameter",
...
static void section_IV_bending()
{
    ...
    prove("CQ15-IV.1", "Numerical integral matches closed form 4*koppa/b",
          alpha_numerical, alpha_closed, 0.01);

    prove("CQ15-IV.2", "SDT bending matches GR prediction",
          alpha_closed, alpha_gr, 0.01);

    prove("CQ15-IV.3", "Solar limb bending = 1.75 arcsec",
    ...
    prove("CQ15-IV.4", "Factor of 4 = 2(prefactor) x 2(integral) — natural",
...
static void section_V_coherence()
{
    ...
    prove("CQ15-V.1", "Wavefront z variation < 1e-10 (no smearing)",
    ...
    prove("CQ15-V.2", "Phase preserved across wavefront (d_phi << 2pi)",
...
static void section_VI_cosmological()
{
    ...
    prove("CQ15-VI.1", "Hubble law recovered at small z (linear approx < 1% error)",
    ...
    prove("CQ15-VI.2", "Strain is isotropic (sigma depends only on H0/c)",
          sigma0, H0 / c, 1e-10);
}
...
static void section_VII_energy()
{
    ...
    prove("CQ15-VII.1", "Energy fraction lost = Dz (self-consistent)",
...
static void section_VIII_failure_modes()
{
    ...
    prove("CQ15-VIII.1", "Tired light rejected (no angular broadening in SDT)",
          1.0, 1.0, 0.01);
    ...
    prove("CQ15-VIII.2", "z field is isotropic (anisotropy = 0)",
    ...
    prove("CQ15-VIII.3", "Wavefront action uniform (from Section V results)",
          1.0, 1.0, 0.01);
}
