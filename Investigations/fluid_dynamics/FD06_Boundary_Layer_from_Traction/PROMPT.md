...
- **Goal.** Add a stationary matter wall to the FD01 relay lattice (half-space `y ≥ 0`, wall at `y=0`) and implement the CQ14 traction coupling on the wall-adjacent spation layer. Demonstrate that the *only* extra rule is the traction grip — no `u=0` is hand-set.
- **Method.** Encode the traction update from `law_VI::traction` (`ω_demand`, `traction_ratio_proton = 12`) as a tangential momentum exchange between wall and first free layer. Initialise the lattice with uniform `u = U` (slip everywhere) and relax. Tag every constant DERIVED / MEASURED-INPUT / CALIBRATED(n) in a parameter ledger (R2). Cite CQ14 (traction), FD01 (NS lattice), FD02 (`ν`) as upstream.
...
**Upstream (required):**
- **CQ14 (Spation traction from the trefoil)** — `law_VI::traction` (`ω_demand`, `traction_ratio_proton = 3(W+1) = 12`, wake channels `ℓ=1/2/≥3`); the mechanical origin of the wall grip and thus of no-slip.
