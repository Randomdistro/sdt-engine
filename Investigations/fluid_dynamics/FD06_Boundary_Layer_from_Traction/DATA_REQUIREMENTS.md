...
External data is used **only** as the convergence target / falsifier in Phases 2–5. None of it enters the derivation as a tunable input. The mechanism is SDT-native (CQ14 traction + FD01 relay); measured fluid properties are legitimate *measured-in-measured-units* inputs (whitelist §0). The Blasius numbers themselves are analytic and require no dataset — they are reproduced, never borrowed.
