...
int main()
{
    ...
    const bool p2_match = std::fabs(cs_ceiling - cs_root3) < 1.0;            // == CQ39 BAO c_s
    const bool phase2 = p2_slope && p2_value && p2_match;
    std::printf("\n  CQ39 cross-check: this c/sqrt(3) IS the coupled-lattice (pre-Clearing)\n");
    ...
    std::printf("  Phase 2 verdict: slope=1/3 %s  value(4sf) %s  CQ39 %s  ->  %s\n",
    ...
    std::printf("    c/sqrt(3)    = %.6e m/s  (= CQ39 BAO sound speed)\n", cs_ceiling);
