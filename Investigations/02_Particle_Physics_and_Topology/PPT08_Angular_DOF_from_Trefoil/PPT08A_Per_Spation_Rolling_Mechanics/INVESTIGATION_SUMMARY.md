# PPT08A INVESTIGATION SUMMARY — Per-Spation Rolling and Gear Mesh Geometry
...
**Investigation**: PPT08A — Per-Spation Rolling and Gear Mesh Geometry  
...
This **exactly matches the target formula from FLM03** to machine precision (error < 10⁻¹⁵).
...
|---------------|------|--------|
| **PPT01** | Trefoil (2,3) knot geometry, aspect ratio | ✓ INPUT |
| **PPT06** | Traction mechanism, surface velocity v₀ | ✓ INPUT |
| **FLM03** | Target formula v(r) = v₀√(R_p/r) | ✓ VALIDATED |
...
|---------------|-------|--------|
| **FLM03** | Rotation field derivation | ✓ RESOLVED — Coefficient now derived [RETRACTED — the 'derivation' was the frozen-radius no-op above; FLM03's coefficient remains a candidate, not derived.] |
| **APS01** | Emissions mechanism | Enhanced — Field now grounded in mechanics |
| **GOM02** | k-hierarchy, bonding | Enhanced — Velocity field at all scales |
...
FLM03 flagged the coefficient `a/r` as CANDIDATE but noted: *"The actual bottom of the stack — the **per-contact, pressure-resolved, packing-aware spation mechanics** — has not been computed in any investigation."*

**PPT08A delivers exactly that**: A complete, from-first-principles derivation of per-spation rolling mechanics that shows:
...
This closes the foundational gap in the rotation field and places FLM03's result on solid mechanical ground.
...
**PPT08A Result** (suitable for citations and downstream use):
...
**For FLM03 users**: The macro formula `v(r) = v₀√(R_p/r)` is now **grounded in per-spation mechanics**, not just a phenomenological fit. The coefficient `a/r` is **derived, not calibrated**.
