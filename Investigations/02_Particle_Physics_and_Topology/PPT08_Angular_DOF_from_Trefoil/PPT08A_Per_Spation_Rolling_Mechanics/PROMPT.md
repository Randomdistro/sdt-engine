...
1. **What don't we know?** — [PPT08A] PPT08A: Per-Spation Rolling and Gear Mesh Geometry: *(unfilled by the 2026-06-27 bulk template upgrade — write the single measurable gap before the next run)*
...
Derive the **per-contact lag coefficient** `a/r` from first principles using Planck-scale spation rolling mechanics. This coefficient is the missing piece in FLM03's macro velocity field.

---

## QUESTION

The rotation field `v(r) = v₀√(R_p/r)` has a rigorously proven **form** (from geometric arguments in FLM03), but the **coefficient** of the per-contact lag `a/r` was marked CANDIDATE (not derived). 
...
```
[RETRACTED — prompt must not contain its verdict; this is a frozen-radius no-op, see PPT08A_VERDICT.md banner]
...
|---------------|------|--------|
| **PPT01** | Trefoil geometry, v_T/v_P | INPUT ✓ |
| **FLM03** | Target formula v(r) | OUTPUT ✓ |
| **PPT06** | Traction mechanism | INPUT ✓ |
...
1. ✓ **Coefficient derivation**: RESOLVED (k=1.0, λ=a/r) [RETRACTED — prompt must not contain its verdict]
2. ⚠ **Latitude verification**: Pending (solver bug, needs correction)
3. ✓ **Macro field match**: CONFIRMED (error < 10⁻¹⁵) [RETRACTED — prompt must not contain its verdict]
4. ✓ **No approximation check**: PASSED (all errors < 1%) [RETRACTED — prompt must not contain its verdict]
...
> [RETRACTED — prompt must not contain its verdict; converted to historical note, see PPT08A_VERDICT.md banner]
...
**Classification**: **CLASS A (PASS)** [RETRACTED — prompt must not contain its verdict]
...
**Downstream Impact**:
- Confirms FLM03's macro formula is grounded in per-spation mechanics
...
- FLM03: Spation Rotation Kinematic Map
- PPT06: Spation Traction from Trefoil
...
*PPT08A · upgraded 2026-06-27 · execute with `PROMPT_EXECUTION_PROTOCOL.md`.*
