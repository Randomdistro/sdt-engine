...
**APS05 cross-link (2026-06-24) — α reframed from "needs e" to a measured anchor with a native mechanism:** the companion investigation [APS05](../../04_Atomic_Physics_and_Spectroscopy/APS05_Alpha_from_Fine_Gross_Ratio/) reads α two independent ways from real hydrogen spectra — as the **fine/gross structure ratio** `α² = 16·Δν_2P/cR∞` → 1/α = 136.91 (no m_e, e, or h), and as the ground-state rung velocity `α = √(2E_ion/m_ec²)` → 1/α = 137.07 — and shows the **α²-scaling of fine structure is native to the movement budget** (`1 − √(1−α²) = ½α²`, matched to 0.0013%), not borrowed Dirac spin-orbit. This **upgrades PPT02's residue**: from *"α requires e as an input"* to **"α is a directly measured spectroscopic anchor (read like m_p/m_e off a mass spectrometer) with a native α²-mechanism; its geometric value is a legitimate OPEN target (E51 / OP-3), not seed-wall-blocked."** The sharpening: because α is *dimensionless*, the FLM06 seed wall that makes `ℓ_P` and `m_e` irreducible does **not** apply to it — so α-from-geometry is unpaid-but-allowed, and the genuinely irreducible dimensional seed is **`e` itself** (EMC02, the `[A·s]` unit-bridge), not α. The negative result here (α not derivable from W=1 topology *in this tool*) stands unchanged.
