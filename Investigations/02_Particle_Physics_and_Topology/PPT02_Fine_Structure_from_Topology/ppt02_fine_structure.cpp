 ...
 * @brief PPT02: Fine Structure Constant from Topology
 *
 * THE QUESTION:
 ...
 *
 * THE CONJECTURE:
 *   α = v_P / v_T at W=1? PPT01 showed v_P/v_T = 1 for (1,1).
...
static void section1_identities()
{
    ...
    prove("PPT02-1d", "r_e = alpha * hbar/(m_e c)",
          r_e_check, r_e, 0.01);

    // Identity 5: g_proton = R_p m_p c / ℏ ≈ 4 = W+1
    double g_p = R_p * m_p * c / hbar;
    prove("PPT02-1e", "g_proton = R_p m_p c / hbar ≈ 4 = W+1",
          g_p, 4.0, 0.1);
...
//
//  The PPT01 mode equations give v_P/v_T = 1 for (1,1).
...
static void section2_geometric_alpha()
{
    ...
    prove("PPT02-2a", "g_electron = alpha",
          g_e, alpha, 0.001);
...
//
//  From EMC01: the transfer function f = P_eff/P_conv couples the
...
static void section3_derivation_attempt()
{
    ...
    prove("PPT02-3a", "alpha from g-ratio",
    ...
    prove("PPT02-3b", "alpha from Bohr radius",
    ...
    prove("PPT02-3c", "alpha from P_eff",
    ...
    //
    // From EMC01: f = 4αℏc l_P / (π R_p² r_e² R_CMB u_CMB)
    ...
    // From PPT01: the torus geometry fixes the RATIO of these.
    // m_p/m_e = 1836.15
    ...
    prove("PPT02-3d", "alpha = 4 / (R_p/r_e × m_p/m_e)",
...
static void section4_sensitivity()
{
    ...
    // From PPT01: the mode equations give analytic solutions for each (p,q)
...
static void section5_what_needed()
{
    ...
    std::puts("  This is the FLM05 investigation (Topological Charge Quantisation).");
    std::puts("");
    ...
    std::puts("    Reducing to 3 inputs requires deriving e from lattice topology (FLM05).\n");
...
{
    std::puts("================================================================");
    std::puts("  PPT02: FINE STRUCTURE CONSTANT FROM TOPOLOGY");
    ...
    std::puts("    Reducing to 3 inputs requires FLM05: Topological Charge Quantisation.\n");
