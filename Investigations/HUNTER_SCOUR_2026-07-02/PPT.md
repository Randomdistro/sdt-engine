...
| **PPT03** | `INVESTIGATION.md`: "**8/8 PASS**, PARTIALLY RESOLVED" | **CIRCULAR padding (labelled honestly)** | Ran it: 8/8 genuinely pass, but 3 are IDENTITY-PASSES at 0.0000 % — `V_disp_p/V_disp_e=m_p/m_e`, `m_p from V_disp_p`, `m_e from V_disp_e` are equal **by definition of Law IV** (re-exposing laws.hpp). The one real number, 6π⁵ (0.0019 %), IS correctly flagged OPEN in prose ("derivation from first principles is OPEN"). Headline "8/8" is padded but the write-up does not overclaim the mechanism. | MINOR |
...
| **PPT07 (ROOTSIM)** | `ROOTSIM_VERDICT.md`: 6π⁵ status | none (gold-standard audit) | Retracts its own earlier "[KILLED]" overreach; runs the 6π⁵ test **blind** (target never enters a computation); grades it **`[OPEN — privileged, unexplained, 19-ppm]`**, not derived, not killed; explicitly **withdraws** any proposal to relabel `laws.hpp`; canon untouched. Matches the known/approved status. | — |
...
- **PPT03** — Mass ratio: **MINOR** — 8/8 padded with 3 Law-IV identity-passes; 6π⁵ correctly OPEN.
