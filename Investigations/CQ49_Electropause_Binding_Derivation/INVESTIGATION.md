# CQ49 — The Electropause: the electron's binding (and P_eff) derived from kinematics
...
> as the kinematic k-rung (the chi of [[CQ03]]), not as the Coulomb coupling. **α remains the one
...
Coulomb force ([[CQ01]] force law, class C). But the magnitude rides on **P_eff**, and P_eff has always
been the weak joint: the engine sets it by `P_eff = 4 k_e e² / (π R_p² r_e²)` — i.e. *calibrated to the
measured Coulomb number* — which is why [[CQ01]] flagged the coefficient as **class E** and the
...
2. **Can that force — hence P_eff — be obtained without ever writing down a charge?** If yes, the CQ01
...
it sits at a **pressure-balance radius**, exactly as the Oort cloud sits at lumiostasis ([[CQ43]]) and
...
is *not* borrowed from QM — it is the hydrogen rung of the koppa ladder, [[CQ03]]). Then:
...
## 3. Inverting the occlusion law derives P_eff — the CQ01 closure **[STRONG, checked]**
...
**Closes:**
- The [[CQ01]] coefficient circularity: P_eff is derived from the electropause balance, not calibrated
  to Coulomb. The delete-test passes.
- The [[CQ47]] **"two-locks"** step: earning the P_eff magnitude from kinematics was the named open
  item; it is done.
...
- **α is still the one input.** The derivation reads α as the kinematic rung (v/c at the H ground
  ...
  argument circular. On SDT's own terms (α = koppa rung, [[CQ03]]) it is not — but the model **does not
...
```bash
# MSVC
cl /std:c++20 /EHsc /O2 /I Engine\include /Fe:cq49.exe \
   Investigations\CQ49_Electropause_Binding_Derivation\cq49_electropause.cpp && cq49.exe
# GCC/Clang
g++ -std=c++20 -IEngine/include \
   Investigations/CQ49_Electropause_Binding_Derivation/cq49_electropause.cpp -o cq49 && ./cq49
```
...
- **[[CQ01]]** — the occlusion force law (C) and the P_eff coefficient (was E); this closes the E.
- **[[CQ33]]** — the P_eff = 4αℏc/(πR_p²r_e²) closed form, here re-derived from the electropause.
- **[[CQ47]]** — repulsion = inverse of convergence; the P_eff magnitude was its "two-locks" open step.
- **[[CQ03]]** — α as the hydrogen koppa rung (χ = 137), not a magic constant.
