...
**Scope:** every formula-bearing file in `sdt-engine` — Engine headers, Theory/Laws/Papers/manuscripts, ATOMICUS, Hubble, Experiments (E01–E101), Investigations (CQ01–CQ26 + E-folders), Benchmarks.
...
| 9 | `⟨cos²θ⟩ = 1/3` (cubic lattice) | Geometric origin of the 1/3 factor | CQ08 closure | Investigations/CQ08.../cq08_measure_closure.cpp:382 | DERIVED | T0 |
...
| 24 | `f = P_eff / P_conv ≈ 2.125e-17` | Transfer function (atomic↔cosmological) | laws.hpp:277; CQ01 cq01_transfer_function.cpp:457 | COMPUTED | T1 |
...
| 35 | `δ = a·t_P/c = a ℓ_P/c²` | Granular Pulse Invariant (acceleration → pulse asymmetry) | CQ08 cq08_granular_pulse_mechanics.cpp:179 | DERIVED | T1 |
| 36 | `F = [Φ V_disp/(3ℓ_P³c²)]·a = m a` | Newton II from relay permeation | CQ08 cq08_local_update_theorem.cpp:399 | DERIVED | T1 |
...
| 56 | `c = v_*√(6πR/(δφ·a(1−e²)))` | **c derived from Mercury precession geometry alone** | Papers/Deriving_c_From_Mercurial_Precession.md:125; CQ24 cq24_koppa_closure.cpp:137 | COMPUTED | T1 |
| 57 | `g(r) = v_surf² R·f_enc(r)/r²` | Interior gravity from enclosed-mass fraction (E97: 1.82% RMS) | laws.hpp:558; E97 e97_seismic.cpp:337 | DERIVED→CONFIRMED | T1 |
...
| 66 | `R_wake = (W+1) ℏ/(m c)` | W+1 wake-radius rule | laws.hpp:716; CQ05 cq05_mass_ratio.cpp:83 | DERIVED | T1 |
...
| 69 | `g(W) = R_wake/ƛ_C ; g(1)=α, g(3)=4` | Wake-to-quantum ratio | laws.hpp:757; CQ03 cq03_fine_structure.cpp:73 | DERIVED | T2 |

## PART X — Atomic physics (T2)

| # | Formula | Meaning | Source | Cert | Rel |
|---|---|---|---|---|---|
| 70 | `E = Ry Z²(1/n₁² − 1/n₂²)` ; `R∞ = m_e c α²/(2h)` | Rydberg transition energy; R∞ derived | laws.hpp:633; CQ12 cq12_forward_derivation.cpp:174 | DERIVED | T2 |
...
| 75 | `v = √(2 E_ion/m_e) ; k = c/v ; zk²=1` | Atomic kinematics closure (per element) | ATOMICUS structural_analysis.cpp:200; CQ12 cq12_forward_derivation.cpp:406 | DERIVED | T1 |
| 76 | `ΔE_HF = (8/3)α⁴ m_e c²(m_e/m_p)μ_P → 21 cm` | Hydrogen hyperfine splitting (1.42 GHz) | CQ09 cq09_spation_rotation_map.cpp:441 | COMPUTED | T2 |

## PART XI — Nuclear construction grammar (ATOMICUS) (T1)

| # | Formula | Meaning | Source | Cert | Rel |
|---|---|---|---|---|---|
| 77 | `n_t = A − 2Z` ; `n_d = 3Z − A − 2` | Alpha-core grammar (1α + n_d deuterons + n_t tritons) | nuclear.hpp:131; ATOMICUS rules; CQ25 | DERIVED | T1 |
...
| 84 | `R_nuc ≈ Z·R_p` (R_He = 2R_p, 0.43%) | Nuclear charge-radius linear-Z scaling | laws.hpp:671; E33 | DERIVED | T2 |
...
| 86 | `B(d) = 2.224 MeV` from `F=(π/4)P_eff R₁²R₂²/r²` | Deuteron binding from occlusion (E29) | E29 PROMPT.md:14 | PENDING | T2 |
| 87 | `B(α) = 28.296 MeV` (tetrahedral 2p+2n occlusion) | Alpha binding from packing (E30) | E30 PROMPT.md:23 | PENDING | T2 |
...
| 90 | `m_n − m_p = 1.293 MeV` (extra trefoil circulation) | Neutron–proton mass difference (E39) | E39 PROMPT.md:20 | PENDING | T2 |

## PART XIII — Cosmology (redshift, BAO, H₀, age, compact objects) (T2)

| # | Formula | Meaning | Source | Cert | Rel |
|---|---|---|---|---|---|
| 91 | `1 + z = exp(κ₀ d), κ₀ = H₀/c` | Pressure-gradient/strain redshift in static lattice | Hubble zk2_lattice_inference.cpp:91; CQ15 cq15...:585 | DERIVED | T1 |
...
| 94 | `BAO scale = c_s·t_coupled ≈ 147 Mpc` | BAO as pre-Clearing lattice standing wave | cosmology.hpp:71; E45 | DERIVED | T2 |
...
| 100 | `a₀ = c H₀/(2π) ≈ 1.04e-10 m/s²` | Derived Milgrom/convergence-floor acceleration | galactic.hpp:76; CQ26 | DERIVED | T1 |
...
| 105 | `g_obs = g_bar/(1−e^(−√(g_bar/a₀)))` ; deep `v⁴=GM_bar a₀` (BTFR slope 4) | Convergence-floor transition disk law | E46 e46_twin_regime.cpp:102 | DERIVED | T1 |
| 106 | `B = Φ(ξ), ξ = g_self/a₀` | Universal drafting crossover (scale-invariant coupling) | CQ26 cq26_drafting_crossover.cpp:54 | COMPUTED | T1 |
| 107 | `τ = κ_area·Λ/m_p ; f_occ = ½(1−e^(−τ)) ; v = v_lim√(2f_occ)` | 3D volumetric eclipse-saturation (1 fitted κ) | galactic.hpp:1426; E46 e46_sparc_rotation_curves.cpp | CALIBRATED | T2 |
...
| # | Formula | Meaning | Source | Cert | Rel |
|---|---|---|---|---|---|
| 117 | `v(r) = cα√(a₀/r) = (c/ϟ_H)√(R_p/r)` ; `k(r)=(1/α)√(r/a₀)` | Single spation-rotation field: 1.831c @ R_p → αc @ a₀ | CQ09 cq09_spation_rotation_map.cpp:113; SPATIAL...PAPER:354 | DERIVED | T1 |
...
| 119 | `koppa_H = α²a₀ = α·ƛ_C = r_e` | Koppa identity: H c-boundary = classical electron radius | CQ16 cq16_k_hierarchy.cpp:192 | DERIVED | T1 |
| 120 | `δ(r)=g₀(R_p/r)² ; g(r)=g₀(1−(R_p/r)²)` | Lattice gap reopening → 1/r² gravity | CQ10 cq10_lattice_pulse.cpp:116 | DERIVED | T1 |
| 121 | `E(L) = σ_string·L` ; `L_c = 2m_π c²/σ` | Trefoil linear confinement & string breaking (QCD-like) | CQ11 cq11_trefoil_confinement.cpp:264; SPATIAL...PAPER:324 | COMPUTED | T1 |
| 122 | `ω_demand/ω_max = 3(W+1) = 12` | Trefoil spation traction ratio (6π winding) | CQ14 cq14_spation_traction.cpp:171; SPATIAL...PAPER:278 | DERIVED | T1 |
| 123 | `ω_p/ω_e = 3(m_p/m_e)(1/α) ≈ 10⁸` | Nuclear→atomic gear ratio | CQ14 cq14_spation_traction.cpp:272; SPATIAL...PAPER:296 | DERIVED | T2 |
| 124 | `z(r)=ϟ/r ; n(r)=1/√(1−2ϟ/r) ; zk²=1` | Spation-depth scalar field → all GR observables | CQ15 cq15_spation_depth_engine.cpp:57 | DERIVED | T1 |
| 125 | `K(P) = K₀ + K'₀ P, K'₀ = d(1+τ) = 3(1+1/3) = 4` | Seismic bulk-modulus pressure derivative from τ=1/3 (E97) | E97 e97_seismic.cpp:457 | DERIVED | T2 |
| 126 | `Ω = √(c²(ϟ₁+ϟ₂)/a³) ; F₁₂ = c²ϟ₁ϟ₂/D²` | Two/three-body orbital dynamics & Lagrange points from koppa | E98 e98_lagrange.cpp:151; Papers/an_argument...:301 | DERIVED | T2 |
...
## PART XVII — Falsifiable predictions, E01–E101 (T2–T3, condensed)
...
| E28–E40 | Hoyle from α+4d packing; `B(d)=2.224`, `B(α)=28.296 MeV`; `R=Z·R_p`; `m_n−m_p=1.293 MeV`; μ_p,μ_n from trefoil | PENDING |
...
| E64–E69 | `L_Sun` from convergence; main sequence = k-gradient; Chandrasekhar 1.44 M☉; TOV 2.2 M☉; no singularity; Hawking `T=ℏc³/8πk_B GM` | — |
| E76 | `G = c²R/(k²r)` — G derived, may vary with environment | — |
| E97 | Earth interior g/P/v_p from koppa+density, **1.82% RMS, no G/M** (CONFIRMED) | CONFIRMED |
| E91,E100 | Muon g−2 from lattice; `c=c(ω)` lattice dispersion (GRB timing) | — |
| E101 | Solar-system convergence budget Σ balances CMB influx (Law II) | — |
...
| Hubble | 46 | ~60 |
| Experiments E01–E101 | 91 | ~95 |
| Investigations CQ01–CQ17 + Atmospheric_Spallation | 36 | ~50 |
| Investigations CQ18–CQ26 + E-folders + loose | ~60 | ~75 |

**Notes:** `Atmospheric_Spallation`, `CQ18_ZOA_Lensing`, and several `E##` investigation folders are methodology/spec only (no `.cpp`, no closed-form formula). Per-isotope (`isotopes/*.md`) and per-element (`elements/*.md`) files instantiate the templates in Parts XI–XII once each; they are deduped here, not enumerated. `m_p/m_e` has **three** distinct SDT derivations (Law VI trefoil #62, `6π⁵` #62b, and the 28D Pappus form CQ17) — all ≈1836.1. `Theory_SCT/`, `Compiler/`, and `Release/` are out of scope (see header).
