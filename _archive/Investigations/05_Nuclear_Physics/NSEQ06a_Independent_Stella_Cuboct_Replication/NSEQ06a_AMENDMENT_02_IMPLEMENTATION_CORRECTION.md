# NSEQ06a Amendment 02 — Implementation Correction Before Acceptance
...
|---|---|---|
| NSEQ06a cuboct direction generator | Constructed the expected twelve directions with an invalid conditional index mapping, so a true cube-edge-midpoint equality was reported as F2 `FAIL`. | Construct each direction by assigning the two signs to the two non-zero coordinate positions. |
...
the NSEQ06a result report. This amendment narrows the prior proposed correction:
