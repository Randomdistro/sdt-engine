// ================================================================
// zk2_lattice_inference.cpp — Lattice Parameter Inference from
//                              Spectral Ratio Observations
//
// NOT a model comparison against ΛCDM.
// NOT importing expansion-based corrections.
//
// Purpose: From observed (z, μ) pairs, infer the lattice
// compression coefficient κ₀, and show whether it is consistent
// with the κ values derived from stellar/planetary observations
// in the other tools (dark_investigation, spation_profile).
//
// Framework:
//   - Redshift is a RATIO: each unit of lattice multiplies
//     photon wavelength by (1 + κ·dl)
//   - Cumulative: 1+z = exp(κ₀ d)  →  d = ln(1+z) / κ₀
//   - Objects are where they are. No comoving distance.
//   - Flux: inverse-square from real physical distance d
//   - Energy loss: tested as both present and absent
//   - No time dilation (static lattice, no expansion)
//
// Three SDT flux hypotheses tested:
//   A. Pure inverse-square: F = L/(4πd²)
//      → d_eff = d = ln(1+z)/κ₀
//   B. Energy-loss: F = L/(4πd²) × 1/(1+z)
//      → d_eff = d × √(1+z)
//   C. Full (1+z): F = L/(4πd²) × 1/(1+z)²  [expansion relic]
//      → d_eff = d × (1+z)   [the old cosplay model]
//
// The DATA decides which hypothesis is correct.
//
// 0.08% tolerance on all numerics.
// ================================================================
...
constexpr double c_mps = 299792458.0;   // m/s
constexpr double R_p_fm = 0.8414;       // proton charge radius (fm, muonic H)
constexpr double Mpc_m = 3.0856775814913673e22;  // metres per Mpc

// ================================================================
// DATA — observed (z, μ, σ) from Pantheon+ SH0ES calibration
// ================================================================
struct SN {
    const char* name;
    double z;
...
std::vector<SN> build_data() {
    return {
        // Hubble flow regime (z > 0.01) — real per-SN
...
// Core: d = ln(1+z) / κ₀,  κ₀ in Mpc⁻¹
// κ₀ = H₀_eff / c_kms  (dimensional bridge to conventional units)

// Model A: pure inverse-square, no energy correction
// d_eff = d = ln(1+z) / κ₀
double sdt_deff_A(double z, double kappa0) {
    return std::log(1.0 + z) / kappa0;
}

// Model B: energy-loss only (photon frequency decreases → less energy)
// d_eff = d × √(1+z)
double sdt_deff_B(double z, double kappa0) {
    return std::log(1.0 + z) * std::sqrt(1.0 + z) / kappa0;
}

// Model C: full (1+z) — the expansion import (for reference)
// d_eff = d × (1+z)
double sdt_deff_C(double z, double kappa0) {
    return std::log(1.0 + z) * (1.0 + z) / kappa0;
}

// Distance modulus: μ = 5 log₁₀(d_eff in Mpc) + 25
...
struct FitResult {
    std::string name;
    double kappa0;   // Mpc⁻¹
    double H0_eff;   // km/s/Mpc  (κ₀ × c, dimensional bridge only)
    double chi2_val;
    double chi2_dof;
    double bic;
    int n_data;
...
FitResult fit_model(const std::vector<SN>& data,
                    ...
                    std::function<double(double, double)> deff_func) {
    ...
    // Search κ₀ ∈ [1e-5, 1e-3] Mpc⁻¹  (H₀ ∈ [3, 300] km/s/Mpc)
...
// ================================================================
// CROSS-REFERENCE: κ from other tools
// ================================================================
struct KappaRef {
    const char* source;
    double kappa_m;       // κ in metres (R × z_surface)
    double R_m;           // radius in metres
    double z_surface;     // surface gravitational redshift
};
...
std::vector<KappaRef> build_kappa_refs() {
    return {
        ...
        {"Proton (H)", 2.817e-15, 5.2918e-11, 5.325e-5},  // classical electron radius
    };
}
...
int main() {
    ...
    // ── SECTION 2: THREE FLUX HYPOTHESES ──
    ...
    auto best = &fits[0];
    for (auto& f : fits) {
        if (f.chi2_val < best->chi2_val) best = &f;
    }
    std::cout << "**Data selects: " << best->name << "**\n\n";

    // ── SECTION 4: RESIDUALS FOR BEST MODEL ──
    ...
    std::cout << "\nCosmological κ₀:\n";
    double kappa0_m = best->kappa0 / Mpc_m;  // convert from Mpc⁻¹ to m⁻¹
    std::cout << "  κ₀ = " << std::scientific << std::setprecision(6) << best->kappa0
              << " Mpc⁻¹\n";
    std::cout << "     = " << kappa0_m << " m⁻¹\n";
    std::cout << "  1/κ₀ = " << std::setprecision(3) << 1.0/kappa0_m << " m  (lattice coherence length)\n\n";

    // Dimensional comparison: κ_sun = 1476.76 m
    // This is the Schwarzschild radius / 2 = GM/c²
    // If cosmological κ₀ is the lattice compression per unit distance,
    // it has dimensions m⁻¹. The stellar κ has dimensions m.
    // They are different quantities — one is intensive (per-distance), 
    // one is extensive (total displacement).
    ...
    // ── SECTION 7: PER-DATAPOINT κ INFERENCE ──
    ...
    int n_pts = 0;
    for (auto& sn : data) {
        // From μ: d_eff = 10^((μ-25)/5) Mpc
        double d_eff = std::pow(10.0, (sn.mu - 25.0) / 5.0);

        // Infer κ₀ from d_eff = ln(1+z) × factor / κ₀
        double factor = 1.0;  // model A
        if (best == &fits[1]) factor = std::sqrt(1.0 + sn.z);
