...
•	Binding Energy: The measured ~2.224 MeV binding energy of the Deuteron is interpreted in SDT as the energy required to overcome this shared-electron bond and separate the two proton vortices. A first-principles calculation of this value from the pressure-well depth is not shown in this document (see the NP17 nuclear-packing investigation for an attempted geometric construction).
3.3. Magnetics:
•	The magnetic moment of the Deuteron is not the simple sum of a proton and neutron moment. It is proposed to be the complex result of the three interacting vortices (two protons, one electron) in their specific geometric configuration. The measured Deuteron magnetic moment (μ<sub>D</sub> ≈ 0.857 μ<sub>N</sub> — μ<sub>N</sub>, the nuclear magneton, is a borrowed unit used here only for comparison to convention; an SDT-native yardstick is owed) is consistent with this picture; a quantitative derivation from the three-vortex geometry is OPEN.
3.4. All Other Properties (Thermodynamics, Electrics, etc.):
•	The chemical properties of Deuterium are nearly identical to Hydrogen because its external electron shell structure is identical. The slight differences in its spectral lines (the isotopic shift, measured) are consistent with SDT's picture of a slightly different recoil for the heavier, more complex Deuteron nucleus; a quantitative derivation of the shift is OPEN.
