...
/// Fixed-step classical RK4 verification route for the same second-order
/// central-acceleration system used by `step`.
///
/// This is intentionally not adaptive: registered comparisons must exercise
/// a declared timestep. As with velocity-Verlet, no stage or final state is
/// committed when a supplied contact boundary is crossed.
[[nodiscard]] inline auto step_rk4(
    std::vector<Body>& bodies,
    double dt_s
) noexcept -> StepStatus {
    if (!std::isfinite(dt_s) || dt_s <= 0.0) {
    ...
    std::vector<Vec3> a1;
    const auto status_1 = accelerations(bodies, a1);
    if (status_1 != StepStatus::advanced) return status_1;

    auto stage_2 = bodies;
    for (std::size_t i = 0; i < count; ++i) {
        stage_2[i].position =
            bodies[i].position + bodies[i].velocity * (0.5 * dt_s);
        stage_2[i].velocity =
            bodies[i].velocity + a1[i] * (0.5 * dt_s);
    }
    std::vector<Vec3> a2;
    const auto status_2 = accelerations(stage_2, a2);
    if (status_2 != StepStatus::advanced) return status_2;

    auto stage_3 = bodies;
    for (std::size_t i = 0; i < count; ++i) {
        stage_3[i].position =
            bodies[i].position + stage_2[i].velocity * (0.5 * dt_s);
        stage_3[i].velocity =
            bodies[i].velocity + a2[i] * (0.5 * dt_s);
    }
    std::vector<Vec3> a3;
    const auto status_3 = accelerations(stage_3, a3);
    if (status_3 != StepStatus::advanced) return status_3;

    auto stage_4 = bodies;
    for (std::size_t i = 0; i < count; ++i) {
        stage_4[i].position =
            bodies[i].position + stage_3[i].velocity * dt_s;
        stage_4[i].velocity =
            bodies[i].velocity + a3[i] * dt_s;
    }
    std::vector<Vec3> a4;
    const auto status_4 = accelerations(stage_4, a4);
    if (status_4 != StepStatus::advanced) return status_4;

    auto next = bodies;
    for (std::size_t i = 0; i < count; ++i) {
        next[i].position += (
            bodies[i].velocity
            + 2.0 * stage_2[i].velocity
            + 2.0 * stage_3[i].velocity
            + stage_4[i].velocity
        ) * (dt_s / 6.0);
        next[i].velocity += (
            a1[i] + 2.0 * a2[i] + 2.0 * a3[i] + a4[i]
        ) * (dt_s / 6.0);
    }

    std::vector<Vec3> final_acceleration;
    const auto final_status = accelerations(next, final_acceleration);
    if (final_status != StepStatus::advanced) return final_status;
