> **⮑ SPEC LINKED — investigation spec written, NOT yet run.**
> Methodology spec exists at `Investigations/05_Nuclear_Physics/NP10_Alpha_Binding_from_Tetrahedral_Occlusion` (PROMPT only — no verdict, no result). This experiment remains an OPEN falsifiable prediction.
> The specification below is the falsifiable prediction.

---

# E30: Alpha Binding from Tetrahedral Occlusion

> **Category**: Nuclear Physics
> **Difficulty**: TRIVIAL
> **Status**: Calculable now

---

## 1. Background

The alpha particle is 2 protons + 2 neutrons in a tetrahedral arrangement. The total binding (28.296 MeV, or 7.074 MeV/nucleon highest of any A≤4 nucleus) should be calculable from the occlusion geometry of four nucleons at their measured positions.

## 2. SDT Prediction

The alpha particle binding energy (28.296 MeV) should emerge from the tetrahedral packing geometry of 2p+2n.

## 3. Standard Prediction

Standard: alpha binding from shell model or cluster model calculations.

## 4. Observable

Tetrahedral occlusion calculation using nuclear radii and the convergent pressure law.

## 5. Apparatus

Computational.

## 6. Feasibility

**TRIVIAL** — calculation.

## 7. Impact

- **If confirmed**: Reproduces the peak light-nucleus stability from geometry alone.
- **If not confirmed**: If the energy disagrees: tetrahedral assumption may need adjustment.
