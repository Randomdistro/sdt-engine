...
| Metric | Count |
|--------|-------|
| Total items parsed | 123 |
| Symbols registered | 65 |
| Equations validated | 79 |
| Derivation steps | 0 |
| Passes | 0 |
| Warnings | 0 |
| Failures | 0 |
...
|----------|---------|---------|--------|
| `$$P{\text{fail}} = \binom{n}{d{\min}} \cdot p{\text{err}}^{d{\min}} \cdot (1 - p{\text{err}})^{n - d{\min}}$$` |  |  | ⏳ |
| `$$\tau = \frac{1}{\lambda \cdot P{\text{fail}}}$$` |  |  | ⏳ |
...
| `S` |  |  | ⏳ |
| `d_\min` |  |  | ⏳ |
| `P_fail = \binomnd_\min  p_err^d_\min  (1 - p_err)^n - d_\min` | 1 | 1 | ✅ |
| `tau = (1)/(\lambda  P_fail)` | 1 | 1 | ✅ |
| `p_err \ll 1` |  |  | ⏳ |
| `tau >= (1)/(\lambda)  ((1)/(p_err))^d_\min` |  |  | ⏳ |
| `R` |  |  | ⏳ |
| `C` |  |  | ⏳ |
| `R < C` |  |  | ⏳ |
| `(k)/(n) < C_E` |  |  | ⏳ |
