...
> **Snapshot:** the working tree is **dirty and mid-refactor** (laws.hpp, benchmarks, theory docs, deleted CQ09, new CQ32–34 all uncommitted; last commit `5669a16` "getting closer"). This audits that snapshot; HEAD differs (see F-1).
...
- **α is an input** (`:114` "Optional empirical anchor", `:122` `alpha_inv`), and `laws.hpp:884` admits α-from-topology *"Would eliminate α as input"* — i.e. **not yet eliminated**. CQ03 "Fine Structure from Topology" itself consumes `alpha_inv` (`cq03_fine_structure.cpp:436`). So α-derivation is **PENDING/exploratory, not closed** — consistent with the open-problem list, but not with a "derived" headline.
