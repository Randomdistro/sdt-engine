...
function invalidateResult() {
  ...
  elements.viewportEmpty.classList.remove("hidden");
...
function renderInspector() {
  ...
  const vectorKeys = {
    position_m: "Position (m)", velocity_m_s: "Velocity (m/s)",
...
async function runCase() {
  ...
  setSolverState("running", "Executing declared fixed steps in WebAssembly worker…");
  setProgress(null);
  try {
    const chunkSteps = Math.max(1, Math.floor(Number(elements.chunkSteps.value) || 1));
    const result = state.sessionStarted && state.result?.status === "running"
      ? await worker.advance(chunkSteps)
      : await worker.run(state.caseDefinition, chunkSteps);
...
function renderCompletedResult() {
  ...
  elements.viewportEmpty.classList.toggle("hidden", frames.length > 0);
...
function renderPairMatrix() {
  ...
  for (const entry of selected) {
    ...
    cell.innerHTML = `<strong>T${entry.target_channel}·S${entry.source_channel}</strong><span></span><span></span>`;
...
function bindEvents() {
  ...
  $("#loadPresetButton").addEventListener("click", () => loadCase(createPreset(elements.presetSelect.value), "Preset loaded."));
  ...
  elements.importInput.addEventListener("change", async () => {
    ...
    if (!file) return;
    try { loadCase(parseCaseJson(await file.text()), `Imported ${file.name}.`); }
...
async function initialize() {
  bindEvents();
  try {
    const shared = caseFromLocationHash(location.hash);
    if (shared) state.caseDefinition = shared;
  } catch (error) {
